    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" class="inline-flex items-center justify-between w-full text-left px-4 py-2 rounded-md focus:outline-none bg-white dark:bg-zinc-900 hover:bg-gray-100 dark:hover:bg-gray-700">
                    {{ __('Request or Complaint') }}
                    <svg class="w-4 h-4 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
                <div x-show="open" @click.away="open = false" class="absolute left-0 z-10 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
                    <div class="py-1">
                        <x-dropdown-link :href="route('file-a-complaint')" wire:navigate>
                            {{ __('Submit') }}
                        </x-dropdown-link>
                        <x-dropdown-link :href="route('track-complaint')" wire:navigate>
                            {{ __('Track') }}
                        </x-dropdown-link>
                    </div>
                </div>
