    ...
    <div class="p-6 text-gray-900">
        <div x-data="{ openTab: 1 }">
            ...
            <div class="flex space-x-4 border-b">
                <button :class="openTab === 1 ? 'border-b-2 font-medium text-blue-500' : 'text-gray-500'"
                    @click="openTab = 1" class="py-2 px-4">
                    Calendar View
                </button>
                <button :class="openTab === 2 ? 'border-b-2 font-medium text-blue-500' : 'text-gray-500'"
                    @click="openTab = 2" class="py-2 px-4">
                    List View
                </button>
            </div>
            @endhasanyrole
            ...
            <div x-show="openTab === 2 || !@json(auth()->user()->hasAnyRole(['superadmin', 'administrator', 'support']))"
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 gap-4 mb-4 md:grid-cols-3">
                        <!-- Date Filter -->
                        <div>
                            <label for="dateFilter" class="block text-sm font-medium text-gray-700">Filter by
                                Date</label>
                        ...
                        <!-- Vehicle Filter -->
                        <div>
                            <label for="vehicleFilter" class="block text-sm font-medium text-gray-700">Filter by
                                Vehicle</label>
                        ...
                        <!-- Status Filter -->
                        <div>
                            <label for="statusFilter" class="block text-sm font-medium text-gray-700">Filter by
                                Status</label>
                            ...
                            <thead>
                                <tr>
                                    <th class="px-6 py-3 text-left bg-gray-50">
                                        <span
                                            class="text-xs font-medium leading-4 tracking-wider text-gray-500 uppercase">Start</span>
                                    </th>
                                    <th class="px-6 py-3 text-left bg-gray-50">
                                        <span
                                            class="text-xs font-medium leading-4 tracking-wider text-gray-500 uppercase">Vehicle</span>
                                    </th>
                                    <th class="px-6 py-3 text-left bg-gray-50">
                                        <span
                                            class="text-xs font-medium leading-4 tracking-wider text-gray-500 uppercase">End</span>
                                    </th>
                                    <th class="px-6 py-3 text-left bg-gray-50">
                                        <span
                                    ...
                                    <th class="px-6 py-3 text-left bg-gray-50">
                                        <span
                                            class="text-xs font-medium leading-4 tracking-wider text-gray-500 uppercase">Destination</span>
                                    </th>
                                    <th class="px-6 py-3 text-left bg-gray-50">
                                        <span
                                            class="text-xs font-medium leading-4 tracking-wider text-gray-500 uppercase">Driver</span>
                                    </th>
                                    @hasanyrole('superadmin|administrator|support')
                                    <th class="px-6 py-3 text-left bg-gray-50">
                                        <span
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($vehicleSchedules as $vehicleSchedule)
                                    <tr class="hover:bg-gray-100 cursor-pointer"
                                        ...
                                        <!-- Start Date -->
                                        <td
                                            class="px-6 py-4 text-sm leading-5 {{ $vehicleSchedule->is_approved ? 'text-gray-900' : 'text-red-500' }}">
                                        ...
                                        <td class="px-6 py-4 text-sm leading-5 text-gray-900">
                                            <div class="flex items-center space-x-2">
                                                ...
                                                <x-secondary-button
                                                    wire:click="$dispatch('openModal', { component: 'modals.vehicleSchedule-modal', arguments: { vehicleSchedule: {{ $vehicleSchedule }} }})">
                                                    <i class="fas fa-pencil-alt"></i>
                                                </x-secondary-button>
                                                @hasanyrole('superadmin')
                                                <x-danger-button x-data
                                                    @click="if (confirm('Are you sure you want to delete this?')) { $wire.call('delete', {{ $vehicleSchedule->id }}) }">
